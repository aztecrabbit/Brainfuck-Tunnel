...
def main():
    ...
    log_connecting = True if len(socks5_port) > 1 else False
    quiet = True if len(socks5_port) > 1 else False

    app.server((inject_host, inject_port), tunnel_type, quiet).start()

    ssh_clients = app.ssh_clients(tunnel_type, inject_host, inject_port, socks5_port, log_connecting=log_connecting)
